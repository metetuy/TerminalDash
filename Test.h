...
void print3DGrid(const std::vector<std::vector<std::vector<int>>> &grid)
{
    for (size_t frame = 0; frame < grid.size(); ++frame)
    {
        std::cout << "Frame " << frame + 1 << ":\n";
        for (size_t row = 0; row < grid[frame].size(); ++row)
        {
            for (size_t col = 0; col < grid[frame][row].size(); ++col)
            {
                cout << (grid[frame][row][col] == 0) ? unoccupiedCellChar : occupiedCellChar;
            }
            std::cout << "\n";
        }
    }
